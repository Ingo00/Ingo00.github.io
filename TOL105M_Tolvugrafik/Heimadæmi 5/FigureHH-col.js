...
var angleIncrement = 2;  // Speed of running
var maxLegAngle = 90;    // Max swing angle for legs
var maxArmSwing = 180;   // Max swing angle for arms (full circular swing)
...
var render = function() {
  ...
  // Synchronize the leg movement (opposite legs move in opposite directions)
  theta[leftUpperLegId] += angleIncrement;
  theta[rightUpperLegId] -= angleIncrement;

  // Synchronize the arm movement
  // Right arm is already correct, now make sure the left arm moves in sync with it
  theta[rightUpperArmId] += angleIncrement;  // Right arm swings forward
  theta[leftUpperArmId] -= angleIncrement;   // Left arm also swings forward (same as right arm)

  // Reverse direction if we hit the maximum swing angle for legs
  if (theta[leftUpperLegId] > maxLegAngle || theta[leftUpperLegId] < -maxLegAngle) {
    angleIncrement = -angleIncrement;  // Change direction of movement for both arms and legs
  }

  // Ensure arms stay within their swing range (-180 to +180)
  if (theta[rightUpperArmId] > maxArmSwing) theta[rightUpperArmId] = -maxArmSwing;
  if (theta[leftUpperArmId] > maxArmSwing) theta[leftUpperArmId] = -maxArmSwing;

  // Update the limb positions
  ...
  // Traverse the figure and render the body
  traverse(torsoId);
