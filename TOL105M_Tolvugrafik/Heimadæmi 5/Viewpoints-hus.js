...
var movement = false;
var origX, origY;
var spinX = Math.asin(100 / 250) * 180 / Math.PI; // Starting vertical angle corresponding to 100 height
var spinY = 90; // Starting horizontal angle for 250 units along X-axis
...
window.onload = function init() {
    ...
    window.addEventListener("keydown", function(e) {
        switch (e.keyCode) {
            ...
            case 56: // 8: from beside the car
                ...
                break;
            case 57: // 9: Above the house (-40, 140), looking at the car
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Ofan á húsi (-40, 140) horft á bílinn";
                break;
            case 38: // up arrow
                height += 2.0;
                document.getElementById("Height").innerHTML = "Viðbótarhæð: " + height;
    ...
    // Mouse handling code for rotation
    ...
    canvas.addEventListener("mousemove", function(e) {
        if (movement) {
            var deltaX = (e.offsetX - origX) * 0.01;
            var deltaY = (e.offsetY - origY) * 0.01;

            spinX += deltaY * 180 / Math.PI;
            spinY += deltaX * 180 / Math.PI;
...
function render() {
    ...
    // Keep the camera 250 units away from the center, starting from the desired position.
    var cameraRadius = 250.0; // Distance from the center of the track
    var cameraX = cameraRadius * Math.sin(radians(spinY)) * Math.cos(radians(spinX));
    var cameraY = cameraRadius * Math.cos(radians(spinY)) * Math.cos(radians(spinX));
    var cameraZ = cameraRadius * Math.sin(radians(spinX));

    // Apply lookAt with the calculated camera position
    mv = lookAt(
        vec3(cameraX, cameraY, cameraZ + height), // Camera position
        vec3(0.0, 0.0, 0.0), // Always look at the center of the track
    ...
    switch (view) {
        ...
        case 8:
            ...
            break;
        case 9:
            // Eye above the house (-40, 140), looking at the car
